...
$event = \mod_sortvoting\event\course_module_instance_list_viewed::create(array(
    'context' => $modulecontext
));
$event->add_record_snapshot('course', $course);
$event->trigger();

$PAGE->set_url('/mod/sortvoting/index.php', array('id' => $id));
...
if (empty($sortvotings)) {
    notice(get_string('no$sortvotinginstances', 'mod_sortvoting'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
foreach ($sortvotings as $sortvoting) {
    if (!$sortvoting->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/sortvoting/view.php', array('id' => $sortvoting->coursemodule)),
    ...
    if ($course->format == 'weeks' || $course->format == 'topics') {
        $table->data[] = array($sortvoting->section, $link);
    } else {
