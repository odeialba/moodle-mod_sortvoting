...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=function(){document.addEventListener("click",(event=>{const saveSortVoteElement=event.target.closest(SELECTORS_SAVEVOTE);saveSortVoteElement&&(event.preventDefault(),function(saveSortVoteElement){saveSortVoteElement.setAttribute("disabled",!0);var sortvotingid=document.getElementsByName("sortvotingid")[0].value,options=document.getElementsByName("option[]"),votes=[];options.forEach((function(option){votes.push({position:option.value,optionid:option.getAttribute("data-optionid")})})),_ajax.default.call([{methodname:"mod_sortvoting_save_vote",args:{sortvotingid:sortvotingid,votes:votes}}])[0].done((function(){window.location.reload()})).fail(_notification.default.exception)}(saveSortVoteElement))}))},_notification=_interopRequireDefault(_notification),_ajax=_interopRequireDefault(_ajax);const SELECTORS_SAVEVOTE="[data-action='savevote']"}));
